...
// utility to shuffle arrays (no TypeScript generics)
...
export function QuizComponent() {
  ...
  const [quizState, setQuizState] = useState('not_started'); // 'not_started' | 'in_progress' | 'finished'
  ...
  const startQuiz = (isContinuing = false) => {
    ...
    if (isContinuing && savedProgress) {
      ...
      setTimeLeft(savedProgress.timeLeft || todayQuiz.timer * 60);
  ...
  const unlockMegaQuiz = () => {
    if (activeQuiz?.megaQuizQuestions) {
      ...
      // Resetting index to continue quiz at the first mega question
  ...
  const finishQuiz = () => {
    ...
    try {
      ...
      console.warn('Failed to save last quiz result:', e);
    }
        ...
        <Card className="w-full max-w-lg text-center shadow-lg">
          ...
          <CardContent>
            <div className="flex flex-col items-center gap-4">
              <p>
                You left off at question {savedProgress.currentQuestionIndex + 1} with{' '}
                {Math.floor(savedProgress.timeLeft / 60)}:
                {(savedProgress.timeLeft % 60).toString().padStart(2, '0')} remaining.
              </p>
            </div>
          </CardContent>
          <CardFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button
              onClick={() => { localStorage.removeItem('quizProgress'); setSavedProgress(null); startQuiz(); }}
              size="lg"
              variant="outline"
              className="w-full"
            >
  ...
  if (quizState === 'in_progress' && questions.length > 0) {
    ...
    const timeProgress = quizTimer > 0 ? (timeLeft / quizTimer) * 100 : 0;
      ...
      <Card className="w-full max-w-2xl shadow-lg">
        ...
        <CardContent className="space-y-6">
          <div className="flex items-start justify-between gap-2">
            <p className="text-lg sm:text-xl font-semibold text-center min-h-[8rem] flex items-center justify-center flex-grow">
  ...
  // --- FINISHED with optional mega-quiz flow (kept for backwards compatibility) ---
